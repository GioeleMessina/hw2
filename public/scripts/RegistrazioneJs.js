...
function checkUserId(event){
    if(inputID.value==""){
        erroreUser.textContent="Campo obbligatorio";
        erroreUser.classList.add("errore");
        inputID.classList.add("erroreInput");
        errore.userId=true;
    }else if(!/^[a-zA-Z0-9_]{1,15}$/.test(inputID.value)){
      ...
      errore.userId=false;
      fetch(SIGNUP_ROUTE+"/username/"+encodeURIComponent(inputID.value)).then(onResponse).then(checkUserJson); 
    }
...
function checkEmail(event){
    if(inputEmail.value==""){
        erroreEmail.textContent="Campo obbligatorio";
        ...
        errore.email=false;
        fetch(SIGNUP_ROUTE+"/email/"+encodeURIComponent(inputEmail.value)).then(onResponse).then(checkEmailJson);
      }
